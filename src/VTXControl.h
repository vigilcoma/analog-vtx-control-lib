...
public:  
  VTXControl(int vtxMode, int softPin, int responseTimeOut = 1000, bool invert = false, int numtries = 3);
  void flush();
  void waitForInMs(unsigned int ms);
  void dumpHex(const uint8_t* b, size_t n);
...
private:
  ...
  bool pitMode = false;
  bool _ignoreSaCrc = false;
  bool initialized = false;//tramp protocol needs to be initialized, so this var reflects state of Tramp initialization
  uint32_t trampLastPacketUs = 0;
  uint16_t trampConfiguredPowerMw = 0;
  uint16_t trampActualPowerMw = 0;
  uint16_t trampMinFrequencyMhz = 0;
  uint16_t trampMaxFrequencyMhz = 0;
  uint16_t trampMaxPowerMw = 0;
  std::vector<uint16_t> trampPowerValues;
  ...
  bool sa_ignoreCrc() const { return _ignoreSaCrc; }
